...
function Player:load()
    ...
    self.shootSound = love.audio.newSource("assets/sfx/shoot.wav", "static")
    self.jumpSound:setVolume(0.1)
...
function Player:UpdateAnimation(animationName)
    ...
    if (not self.bGrounded or self.animations.shootingTimer < self.animations.shootingTimeEnd) then
        return
    end
...
function Player:keypressed(keyCode)
    ...
    if (keyCode == "space" and self.animations.shootingTimer > 0.25) then
        self:Shoot()
...
function Player:Shoot()
    ...
    Bullet.newBullet(self, 10, posX, self.posY + 63, self.bAnimationReversed, 0.25,
        "assets/player/bulletSpriteSheet.png")
end
