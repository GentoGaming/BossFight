...
function Bullet.newBullet(owner,damage,posX,posY,bReversed,scale,spriteName)
    ...
    nBullet.physics = {}
    nBullet.scale = scale
    if bReversed then nBullet.posX = posX - 20 end
    ...
    nBullet.animations = {}
    nBullet.rotatingBulletGrid = anim8.newGrid(nBullet.width,nBullet.height,nBullet.rotatingBulletprite:getWidth(),nBullet.rotatingBulletprite:getHeight())
...
function Bullet:draw()
    self.animations.rotate:draw(self.rotatingBulletprite,self.posX - self.width / 2 ,self.posY - self.height / 2,nil,self.scale ,self.scale )
end
